...
fn init_run_epochs(
    ...
    SubtensorModule::epoch(netuid, Some(false)); // run first epoch to set allowed validators
    ...
    for _ in 0..epochs {
        if sparse {
            SubtensorModule::epoch(netuid, Some(false));
        } else {
...
fn test_bonds() {
    new_test_ext(1).execute_with(|| {
		...
		if sparse { SubtensorModule::epoch( netuid, Some(false) ); }
